...
class BackupCommand extends BaseCommand {
	...
	protected function uploadS3()
	{
		...
		$s3->putObject( array(
			'Bucket'     => $bucket,
			'Key'        => $this->getS3DumpsPath() . '/' . $this->fileName . '.gz',
			'SourceFile' => $this->filePath . '.gz',
		) );

		// Delete local file
		$this->fs->remove( $this->filePath . '.gz' );
	}
